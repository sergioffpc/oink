...
class SipMessage:
    ...
    def from_string(cls, raw_message):
        ...
        for line in lines[1:]:
            ...
            name = urlparse.unquote(header_match.group(1)).lower()
    ...
    def add_multi_header_from_str(self, name, raw_val):
        ...
        if name == "contact":
            ...
            raise SipParseError("Don't know how to process header {} as a multi-header".format(name))

        # Make sure there's no leftover data after parsing (either we parsed wrong or the header is invalid, either way - bad)
        if data:
            raise SipParseError("Leftover data found after processing {} header".format(name))
    ...
    def add_header_from_str(self, name, data):
    ...
    def stringify(self):
        ...
        if self.type == self.TYPE_RESPONSE:
            msg_str = "SIP/{} {} {}\r\n".format(ver, self.status, self.reason)
        else:
            uri = stringify_uri(self.uri)
            msg_str = "{} {} SIP/{}\r\n".format(self.method, uri, ver)
