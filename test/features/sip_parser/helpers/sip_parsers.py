...
def parse_multiheader(parse_fn, data):
...
def parse_auth_header_with_scheme(data):
...
def parse_cseq(data):
    ...
    return {"seq": int(m.group(1)), "method": urlparse.unquote(m.group(2))}
...
def parse_request(lines):
    ...
    return {
        "method": urlparse.unquote(req_match.group(1)),
